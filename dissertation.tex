...
We need some metric to be able to judge the expected delay of a link, in order to compare it to a stable backup link that we can switch onto when appropriate, to minimise the overall delay. We use a metric based on the uptime history of the link, computing an integral over a window of the history after exponentially weighting it, as we can see in Figure \ref{fig:metric}. Integrating over the uptime history tells us how often the link is up, giving us some idea of its reliability over time. Computing over a window allows us to delete old measurements, saving memory, and the exponential weighting gives more importance to recent events, making it more responsive to changes. The parameters to the exponential weighting allow us to tune its responsiveness.
