...
% JCW: Do we need an abstract?
% Abstarct should be: 2 sentences why, 2 sentences what, 2 sentences results, 2 sentences impact
...
% e.g. \subsection{Delay Tolerant Networking}
...
% JCW: s/The primary goal of this project is to compare/This project compares
% (this pattern comes up a lot --- try to make sentences more active)
...
% JCW : drop Background?  I'm tired of backgrounds by the time I get to here!
...
% JCW: You should refer to these figures from the text --- otherwise the reader won't look at them. (same with your other figures)
...
% JCW: feels wordy
% JCW: My 2c is that this whole thing should be written in the present tense -- rather than referring to the evaluation as the future?
...
% JCW: more exciting!  "However, because CORE .... clock, performance ... is dependent on ..."
% JCW: Also note use of 'I' --- you should be using that all the way through to make this a bit less wordy. (or we if you feel like being academic in style)
% also see note on future tense
...
% jcw not sure how well this flows from last para.
...
A heartbeat message is a UDP packet with only header, no payload. An LSA is a UDP packet whose payload is the sending node's graph representation of the network, simply copied into the payload using \texttt{memcpy}. The graph representation uses statically sized arrays rather than dynamically sized `\texttt{malloc}ed' ones, to make allocation and indexing simpler.
% JCW: hm.  Can you say this in a way that doesn't sound quite so much like you have an unfinished project and simultaneously trashes your evaluation?
While it would be more space-efficient to pack the graph into a more compressed structure for sending over the link, the network topologies that I use in the evaluation are not large enough for this to be an issue. It would be a simple fix if this became an issue.
...
% JCW -- add a sentence -- why both needed?
...
% JCW -- weak to say "as I am more concerned with ..." say instead "as delay is more important than throughput in DT networks" (e.g.).  Not sure where last sentence in this para links in.
...
% JCW:  what is pathfinding!! It's just appeared out of nowhere I think
...
% JCW -- I've stopped following this section and I don't know why -- probably pathfinding would be a good start -- but this also feels like it doesn't link with the last para?
